...
# ------------------------------------------------
docker run --name='gitlab' -d -p 10022:22 -e 'GITLAB_SSH_PORT=10022' -e 'SSL_SELF_SIGNED=true' -e 'VIRTUAL_HOST=gitlab.server.com' -e 'VIRTUAL_PORT=80' -e 'SMTP_USER=user@gmail.com' -e 'SMTP_PASS=password' -v /var/run/docker.sock:/run/docker.sock -v $(which docker):/bin/docker -v /var/docker-data/gitlab:/home/git/data --link postgresql:postgresql --link redis:redisio sameersbn/gitlab:7.3.2-1
